...
def basic_device_readout_errors(properties=None, target=None, path=None, ibmname=None):
    """
    ...
    pattern = str(ibmname)+'readout_error_values.csv'
    filename = find(pattern, path)
    
    colnames = ['prob_meas1_prep0', 'prob_meas0_prep1']
    readout_error_values = pandas.read_csv(filename, names=colnames)
    readout_error_values = readout_error_values.values.tolist()
    
    errors = []
    if target is None:
        if filename is None and properties is None:
            raise NoiseError("Either properties or target or csv file must be supplied.")
        # create from BackendProperties
        for qubit, value in enumerate(readout_error_values):
...
def basic_device_gate_errors(
    ...
    """
    if properties is None and target is None and path is None:
        raise NoiseError("Either properties or target or path for a csv file must be supplied.")
    ...
    if thermal_relaxation:
        ...
        pattern = ibmname+'thermal_relaxation_values.csv'
        filename = find(pattern, path)
        
        colnames = ['T1', 'T2', 'Freqs']
        relax_params = pandas.read_csv(filename, names=colnames)
        relax_params = relax_params.values.tolist()
        
        #relax_params = thermal_relaxation_values(properties, bname)
        ...
        if gate_lengths:
            pattern = ibmname+'gate_length_values.csv'
            filename = find(pattern, path)
            
            colnames = ['gate_name', 'qubits', 'gate_length']
            gate_lengths = pandas.read_csv(filename, names=colnames)
            gate_lengths = gate_lengths.values.tolist()
    ...
    pattern = ibmname+'gate_param_values.csv'
    filename = find(pattern, path)
    
    colnames = ['gate_name', 'qubits', 'gate_length', 'gate_error']
    device_gate_params = pandas.read_csv(filename, names=colnames)
    device_gate_params = device_gate_params.values.tolist()
    ...
    for name, qubits, gate_length, error_param in device_gate_params:
        ...
        QubitArr = []
        qubits = qubits.split('[')
        qubits = qubits[1].split(']')
        if name =='ecr':
            qub= qubits[0].split(',')
            QubitArr = [int(qub[0]), int(qub[1])]
        else:
            QubitArr = [int(qubits[0])]
        ...
        if thermal_relaxation:
            relax_error = _device_thermal_relaxation_error(
                            QubitArr, relax_time, relax_params, temperature
            )
            
        # Get depolarizing error channel
        if gate_error:
            depol_error = _device_depolarizing_error(QubitArr, error_param, relax_error)
        # Combine errors
        combined_error = _combine_depol_and_relax_error(depol_error, relax_error)
        if combined_error:
            errors.append((name, QubitArr, combined_error))
